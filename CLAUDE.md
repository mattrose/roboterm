...
`make app` builds a bundle that still points at the Homebrew stack at runtime (fast, for development). `make bundle` runs `macos/bundle_deps.py` on top of it to copy the Python framework, the GTK4/VTE dylib closure, PyGObject/pycairo, typelibs, GSettings schemas, gdk-pixbuf loaders, gettext message catalogs and the Adwaita icon theme into the `.app`, rewrite every `install name` off `/opt/homebrew`, and ad-hoc codesign — producing a distributable, dependency-free bundle. `launcher.c` detects at runtime whether `Contents/Frameworks/Python.framework` exists and roots all paths (`PYTHONHOME`, `GI_TYPELIB_PATH`, `DYLD_LIBRARY_PATH`, schema/pixbuf/icon dirs) inside the bundle when it does, else falls back to Homebrew — so one launcher serves both build modes. Key bundling subtleties `bundle_deps.py` handles (each learned from a concrete failure): dylibs are named by their versioned install-id soname (so `@rpath/libgtk-4.1.dylib` resolves); the dependency closure follows `@loader_path`/`@rpath` deps too (Homebrew's ICU libs reference siblings relatively — miss this and `libvte` fails to load); typelib shared-libraries are seeded as extra roots (Gtk/Adw/Vte are `dlopen`'d via typelibs, not linked); Homebrew's own `LC_RPATH` entries are stripped (else `@rpath` deps resolve to the Homebrew copy, causing duplicate-type-system clashes); the gdk-pixbuf `loaders.cache` is regenerated from the *original* loaders (querying the relocated copies SIGKILLs on the invalidated signature); Homebrew's `icon-theme.cache` files are deleted after the icon themes are copied (GTK trusts a cache over the directory, and Homebrew's predates the app icon `create_icon.py` installs into `hicolor`, so leaving it makes `Gtk.IconTheme.has_icon` report the icon missing while the PNGs sit right there); and message catalogs need a runtime rebind, not just a copy — each library hardcodes its build-time Cellar path (`/opt/homebrew/Cellar/gtk4/<ver>/share/locale`) as its gettext lookup dir, and that's a plain C string `install_name_tool` can't rewrite, so `copy_locales` stages the catalogs plus a `domains` manifest and `_bind_bundled_locales` in `roboterm/__init__.py` calls `bindtextdomain` for each one *after* `Adw.init()` (GTK's own `bindtextdomain` during init would overwrite an earlier call). Copying without the rebind is inert: the bundle looks like it has translations but still resolves to the Homebrew path, so it silently falls back to English on a clean Mac. Only `LOCALE_DOMAINS` x `LOCALE_LANGS` is staged — Homebrew's tree holds 123 languages at ~190KB each, and shipping all five domains for all of them cost 23MB for catalogs a given bundle will load exactly one of; the default set is ~2.5MB and `ROBOTERM_LOCALES` (`all`, or a comma-separated list) widens or narrows the language half at build time. `glib20` is excluded on purpose — it was ~45% of the bytes and covers low-level GLib/GIO error strings a terminal rarely surfaces, so it falls back to English while everything the user actually reads stays translated.
...
Widgets communicate via custom GObject signals rather than direct references pointing "up" the tree — e.g. `TerminalWidget` emits `split-right`/`split-down`/`close-pane`/`rotate-cw`/`new-tab`/etc. and `PaneManager` (in `panes.py`) is what actually connects to and acts on them (`_make_frame` wires every new terminal's signals). `PaneManager` itself emits `all-closed`/`new-tab`/`new-window`/`title-changed`, which `TerminalWindow` (in `window.py`) connects to per-tab. Titles travel the same way: VTE's `window-title-changed`/`current-directory-uri` become `TerminalWidget`'s `title-changed`, which `PaneFrame` uses for its titlebar and `PaneManager` re-emits for the *active* pane only, so `TerminalWindow` can update that tab's `Adw.TabPage` title and the window title. This means the same user action (e.g. "split right") can be triggered from the VTE context menu, the app menu bar (macOS), or a keybinding, and all three converge on the same signal/action path — when adding a new pane/tab operation, wire it the same way (emit a signal from the leaf widget, handle it in the owning manager) rather than reaching into widget internals.

### Tabs are an Adw.TabView

`TerminalWindow` holds one `Adw.TabView` (the pages) plus an `Adw.TabBar` added as a second `Adw.ToolbarView` top bar (the strip). Each tab is an `Adw.TabPage` whose child is a `PaneManager`, so `_active_panes()` is `get_selected_page().get_child()`. libadwaita owns everything the tab strip draws — label, ellipsizing, close button, reordering, drag-and-drop — and `Adw.TabBar` autohides itself while there is one page, so there is no tab-label widget and no show-tabs toggle to maintain.

The header's `Adw.TabButton` (tab count, opens the grid of thumbnails) and the `Adw.TabOverview` it opens — also reachable as `win.tab-overview` from both menus and as the `tab-overview` keybinding — depend on how the two are nested: the overview must be the window's *direct* child with everything else — header bar, tab bar, tab view — as its `child`, because the button reaches it through the `overview.open` action group the overview inserts into the widgets below itself. Put the button anywhere outside that subtree and it goes inert, with no error. The overview's own "New Tab" button is its `create-tab` signal, whose handler has to return the new `Adw.TabPage` — that is why `_new_tab` returns its page.

A tab can move between windows — dragging one onto the desktop makes `Adw.TabView` emit `create-window`, and `_on_create_window` answers with the tab view of a new, empty window (`TerminalWindow(app, initial_tab=False)`, via `TerminalApp.new_window(with_tab=False)`) for libadwaita to transfer the page into. That mobility is why a tab's `PaneManager` signals are connected in `_on_page_attached` and dropped in `_on_page_detached` rather than in `_new_tab`: a transferred tab arrives carrying handlers that point at the window it left, and would otherwise keep asking that one to close it. For the same reason the "Terminal N" name a tab falls back to when no pane reports a title lives on the `PaneManager` (`default_title`) instead of in a per-window dict — it travels with the tab.

Three more non-obvious bits:

- A closing tab always goes through the `close-page` signal (the × button and `close_page()` alike), and because `_on_close_page` returns `True` it must call `close_page_finish()` itself — synchronously, since nothing asks the user to confirm.
- A window closes with its last tab, from `_on_page_detached`, and that close is deferred to an idle on purpose: a detached page may be mid-transfer into another window, and tearing this widget tree down inside the handler would take the page with it. The app then exits on its own once no windows are left, which is `Gtk.Application` behavior, not ours — so don't reintroduce an explicit `quit()` here; it would kill the other windows too.
- `Adw.TabPage`'s tooltip is parsed as markup, so a shell-set title has to go through `GLib.markup_escape_text` (`_set_page_title` does both halves — pass titles through it rather than `set_title`).

`tests/integration/test_tabs.py` covers all of this, transfers included. Two of its tests need the window actually presented (`Gtk.Window.close()` is a no-op otherwise) and have to pump the main loop by hand — see `_flush` there, which acquires the default main context first, since under pytest no loop has ever run and an unowned context dispatches nothing.
...
1. **Actual key handling** — `TerminalWindow._build_key_table()` (in `window.py`) builds a `{(modifier_mask, keyval): callback}` dict straight from `Settings.get().get_keybindings()`, applied via a `Gtk.EventControllerKey` in the **CAPTURE** phase (so it intercepts before VTE sees the key on every platform). This table rebuilds automatically on `Settings.connect_changed`, which is what makes keybindings live-editable from Preferences. Each binding is entered under every keyval its key can actually arrive as, via `_accel_keyvals`: an accelerator names the *unshifted* key (`<Meta><Shift>backslash`) while the event carries the shifted one (`bar`), so keying the table on the accelerator's own keyval silently breaks every punctuation binding — the tab, rotate and overview defaults all are one. Unlike `Gtk.ShortcutController`, this hand-rolled matching gets no keymap translation for free.
...
Confirmed *not* to fix it **on macOS**, despite looking plausible: resizing the window programmatically, `Gdk.Surface.queue_render()` (even called repeatedly), toggling `win.set_visible(False/True)`. The only thing that reliably works there is a real keystroke or click delivered through the actual OS input pipeline (verified via `System Events` synthetic key events with Accessibility permission) — GLib-timer-driven redraw requests don't go through whatever AppKit display-commit path a real NSEvent does. Subsequent tabs/splits/windows are unaffected since their host window is already receiving real frame-clock ticks. See the "Known Issues" note in README.md.

**Keep the `GSK_RENDERER=cairo` line in `roboterm/__init__.py` — do not remove it as dead weight.** The maintainer reports it fixes this class of blank/unpainted-window problem on platforms other than macOS, which is why it's there. Two caveats to know before touching it:

- The macOS-only testing behind the paragraph above did *not* reproduce a benefit there (8 trials — 4 as a shell env var, 4 in-process — with the blank window still appearing in all but one). That's evidence about macOS specifically, not about the platforms it was kept for, so it is not grounds for deleting the line.
- It is currently gated behind `sys.platform == "darwin"`, so as written it only takes effect on macOS — the one platform where it wasn't observed to help. If you're chasing this bug on another platform, check that gate first; widening it is a deliberate call, since Cairo is software rendering and slower than the default GPU-backed renderer for a terminal doing frequent scrollback/redraw.

If revisiting the macOS case specifically, start from the "real OS input event" finding above rather than re-testing renderers.
...
roboterm/
  ...
  window.py                TerminalWindow (Adw.TabView tabs, menu, key handling)
